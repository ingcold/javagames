...
public class PrototypeEditor extends SimpleFramework{
    ...
    protected void processInput(float delta){
        ...
        mousePos=getWorldMousePosition();//SimpleFramework
        if(mouse.buttonDownOnce(MouseEvent.BUTTON1))polygon.add(mousePos);
        if(mouse.buttonDownOnce(MouseEvent.BUTTON2))closed=!closed;
        if(mouse.buttonDownOnce(MouseEvent.BUTTON3))if(!polygon.isEmpty())polygon.remove(polygon.size()-1);
        if(keyboard.keyDownOnce(KeyEvent.VK_C))polygon.clear();
        if(keyboard.keyDownOnce(KeyEvent.VK_SPACE))printPolygon();
    }
    ...
    private void drawAxisLines(Graphics g){
        ...
        Vector2f button=new Vector2f(0.0f,-top.y);
        drawLine(g,top,button);
    }
    private void drawPolygon(Graphics g){
        g.setColor(Color.BLACK);
        if(polygon.size()==1)drawPoint(g,polygon.get(0));
        for(int i=0;i<polygon.size()-1;++i)drawLine(g,polygon.get(i),polygon.get(i+1));
