...
public class FlyingShipExample extends SimpleFramework{
    ...
    protected void processInput(float delta) {
        ...
        if(keyboard.keyDown(KeyEvent.VK_SPACE))bullets.add(ship.launchBullet());
        //keyDown<-->keyDownOnce,keyDownOnce会中断，只准你按一次
    ...
    protected void updateObjects(float delta) {
        ...
        for(PrototypeBullet bullet:copy){
            bullet.update(delta);
            if(wrapper.hasLeftWorld(bullet.getPosition()))bullets.remove(bullet);
        }//wrapper的重点在于其函数，wrapper作用在于持有各种函数
    }
    ...
    protected void render(Graphics g) {
        ...
        for(PrototypeBullet b:bullets)b.draw((Graphics2D) g,view);
    }
