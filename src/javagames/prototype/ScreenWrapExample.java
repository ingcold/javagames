...
public class ScreenWrapExample extends SimpleFramework{
    ...
    protected void processInput(float delta) {
        ...
        if(mouse.isRelative()){
            ...
            pos=pos.add(v);
        }else {pos=getWorldMousePosition();}
        if(keyboard.keyDownOnce(KeyEvent.VK_SPACE)){
            mouse.setRelative(!mouse.isRelative());
            if(mouse.isRelative())pos=new Vector2f();
        }
    }
    ...
    private Vector2f[] transform(Vector2f[] poly,Matrix3x3f mat){
        ...
        for(int i=0;i<copy.length;++i)copy[i]=mat.mul(poly[i]);
        return copy;
    }
    ...
    protected void render(Graphics g) {
        ...
        for(Vector2f[] toRender:renderList){
            for (int i=0;i<toRender.length;++i)toRender[i]=view.mul(toRender[i]);
