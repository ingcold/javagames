...
public class PrototypeAsteroid {
    public enum Size{
        Large,Medium,Small;
    }//Vector2f[][]类型，定义要在PrototypeAsteroidFactory完善
    ...
    public void setPolygon(Vector2f[] polygon){this.polygon=polygon;}
    public void setPosition(Vector2f position){this.position=position;}
    public Vector2f getPosition(){return position;}
    public void setSize(Size size){this.size=size;}
    public Size getSize(){return size;}
    public void update(float time){//更新参数-->获得transformPolygon-->wrap()
    ...
    public void draw(Graphics2D g,Matrix3x3f view){
        for(Vector2f[] poly:renderList){//对每一个polygon
            for(int i=0;i<poly.length;++i)poly[i]=view.mul(poly[i]);//对polygon的每一个点
    ...
    public boolean contains(Vector2f point){
        for(Vector2f[] polygon:renderList)if(pointInPolygon(point,polygon))return true;
        return false;
    }
    private boolean pointInPolygon(Vector2f point,Vector2f[] polygon){
        ...
        return inside;
    }//pointInPolygon在逻辑上对应inside，true就是true，false就是false
