...
public class PrototypeShip {
    ...
    public PrototypeShip(PolygonWrapper wrapper){//构造函数传进wrapper一个参数
        ...
        renderList=new ArrayList<Vector2f[]>();//属性区只是有某个变量，构造函数才分配内存生成对象
    }
    public void setDamaged(boolean damaged){this.damaged=damaged;}
    public boolean isDamaged(){return damaged;}
    public void rotateLeft(float delta){angle+=rotationDelta*delta;}
    public void rotateRight(float delta){angle-=rotationDelta*delta;}
    public void setThrusting(boolean thrusting){curAcc=thrusting?acceleration:0.0f;}
    //curAcc（实际状况）<---acceleration（预备参数）
    ...
    public void update(float time){
        ...
        wrapper.wrapPolygon(world,renderList);//if(~)renderList.add(wrap~(poly))
    }
    ...
    private void updatePosition(float time){//time-->delta
    ...
    public void draw(Graphics2D g,Matrix3x3f view){
        ...
        for(Vector2f[] poly:renderList) {
            for (int i = 0; i < poly.length; ++i) poly[i] = view.mul(poly[i]);
