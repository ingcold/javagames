...
public class CannonExample extends JFrame implements Runnable {
    ...
    private void initialize(){
        ...
        for(int i=0;i<cannon.length;++i)cannon[i]=scale.mul(cannon[i]);
    }
    ...
    private void processInput(double delta){
        ...
        if(keyboard.keyDown(KeyEvent.VK_A))cannonRot+=cannonDelta*delta;
        if(keyboard.keyDown(KeyEvent.VK_D))cannonRot-=cannonDelta*delta;
        if(keyboard.keyDownOnce(KeyEvent.VK_SPACE)){//空格键发射bullet
    ...
    private void render(Graphics g){
        ...
        for(int i=0;i<cannon.length;++i)cannonCpy[i]=viewport.mul(cannonCpy[i]);
    ...
    private void drawPolygon(Graphics g,Vector2f[] polygon){//广泛统一的巧妙方法
        Vector2f P;
    ...
    public static void main(String[] args){
        final CannonExample app=new CannonExample();//CV过来的时候记得把app修改一下
