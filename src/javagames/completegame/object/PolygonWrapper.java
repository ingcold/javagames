...
public class PolygonWrapper {
    ...
    public void wrapPosition(Vector2f[] poly,Vector2f positon,List<Vector2f> centerList){
        Vector2f min=getMin(poly);
        Vector2f max=getMax(poly);
        boolean north=max.y>worldMax.y;
        boolean south=min.y<worldMin.y;
        boolean west=min.x<worldMin.x;
        boolean east=max.x>worldMax.x;
        if(west)centerList.add(positon.add(new Vector2f(worldWidth,0.0f)));
        if(east)centerList.add(positon.add(new Vector2f(-worldWidth,0.0f)));
        if (north)centerList.add(positon.add(new Vector2f(0.0f,-worldHeight)));
        if(south)centerList.add(positon.add(new Vector2f(0.0f,worldHeight)));
        if(north&&west)centerList.add(positon.add(new Vector2f(worldWidth,-worldHeight)));
        if(north&&east)centerList.add(positon.add(new Vector2f(-worldWidth,-worldHeight)));
        if(south&&west)centerList.add(positon.add(new Vector2f(worldWidth,worldHeight)));
        if(south&&east)centerList.add(positon.add(new Vector2f(-worldWidth,worldHeight)));
    }
