...
public class Vector2f {
    ...
    //void类型的都是改变自身，Vector2f类型的会返回Vector2f
    ...
    public Vector2f mul(float scalar){return new Vector2f(scalar*x,scalar*y);}
    public Vector2f div(float scalar){return new Vector2f(x/scalar,y/scalar);}
    public Vector2f inv(){return new Vector2f(-x,-y);}
    public Vector2f norm(){return div(len());}
    //下面是Vector2f这个类里的第三类函数
    public float dot(Vector2f v){return x*v.x+y*v.y;}
    public float len(){return (float) Math.sqrt(x*x+y*y);}
    public float lenSqr(){return x*x+y*y;}
    public Vector2f perp(){return new Vector2f(-y,x);}
    public float angle(){return (float) Math.atan2(y,x);}//atan一个参，atan2二个参
    public static Vector2f polar(float angle,float radius){return new Vector2f(
