...
public class ScaleImageExample extends SimpleFramework{
    ...
    protected void initialize() {
        ...
        for(int i=0;i<100;++i)generateAveragedInstance();
        long end=System.nanoTime();
        averagedSpeed=(end-start)/1.0E6;
        averagedSpeed/=100;
        System.out.println("Generating Nearest Neighbor Image");
        start=System.nanoTime();
        for(int i=0;i<100;++i)generateNearestNeighbor();
        end=System.nanoTime();
        nearestSpeed=(end-start)/1.0E6;
        nearestSpeed/=100;
        System.out.println("Generating Nearest Neighbor 2");
        start=System.nanoTime();
        for(int i=0;i<100;++i)nearest2=scaleWithGraphics(RenderingHints.VALUE_INTERPOLATION_NEAREST_NEIGHBOR);
        end=System.nanoTime();
        nearest2Speed=(end-start)/1.0E6;
        nearest2Speed/=100;
        System.out.println("Generating Bilinear");
        start=System.nanoTime();
        for(int i=0;i<100;++i)bilinear=scaleWithGraphics(RenderingHints.VALUE_INTERPOLATION_BILINEAR);
        end=System.nanoTime();
        bilinearSpeed=(end-start)/1.0E6;
        bilinearSpeed/=100;
        System.out.println("Generating Bicubic");
        start=System.nanoTime();
        for(int i=0;i<100;++i)bicubic=scaleWithGraphics(RenderingHints.VALUE_INTERPOLATION_BICUBIC);
        end=System.nanoTime();
        bicubicSpeed=(end-start)/1.0E6;
        bicubicSpeed/=100;
        System.out.println("Generating Step Down Bilinear");
        start=System.nanoTime();
        for(int i=0;i<100;++i)stepDownBilinear=scaleDownImage(RenderingHints.VALUE_INTERPOLATION_BILINEAR);
        end=System.nanoTime();
        stepDownBilinearSpeed=(end-start)/1.0E6;
        stepDownBilinearSpeed/=100;
        System.out.println("Generating Step Down Bicubic");
        start=System.nanoTime();
        for(int i=0;i<100;++i)stepDownBicubic=scaleDownImage(RenderingHints.VALUE_INTERPOLATION_BICUBIC);
        end=System.nanoTime();
    ...
    private void createTestImage(){
        ...
        g2d.drawLine(sprite.getWidth()/2,sprite.getHeight(),0,sprite.getHeight());
        //draw circle
        g2d.drawOval(0,0,sprite.getWidth(),sprite.getHeight());
        //draw hash lines
        g2d.setColor(Color.GREEN);
        int dx=sprite.getWidth()/18;
        for(int i=0;i<sprite.getWidth();i+=dx)g2d.drawLine(i,0,i,sprite.getHeight());
        g2d.setColor(Color.BLUE);
        dx=sprite.getHeight()/18;
        for(int i=0;i<sprite.getHeight();i+=dx)g2d.drawLine(0,i,sprite.getWidth(),i);
        //gradient circle
    ...
    protected void render(Graphics g) {
        ...
        time=String.format("%.4 ms",bicubicSpeed);
