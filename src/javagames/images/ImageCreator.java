...
public class ImageCreator extends SimpleFramework{
    ...
    protected void processInput(float delta) {
        ...
        if(keyboard.keyDownOnce(KeyEvent.VK_1))createFile("jpg","image-creator.jpg");
        if(keyboard.keyDownOnce(KeyEvent.VK_2))createFile("bmp","image-creator.bmp");
        if (keyboard.keyDownOnce(KeyEvent.VK_3))createFile("gif","image-creator.gif");
        if(keyboard.keyDownOnce(KeyEvent.VK_4))createFile("png","image-creator.png");
        if(keyboard.keyDownOnce(KeyEvent.VK_5))loadFile("image-creator.jpg");
        if(keyboard.keyDownOnce(KeyEvent.VK_6))loadFile("image-creator.bmp");
        if (keyboard.keyDownOnce(KeyEvent.VK_7))loadFile("image-creator.gif");
        if (keyboard.keyDownOnce(KeyEvent.VK_8))loadFile("image-creator.png");
    }
    private void createFile(String type,String fileName){
        try {
            sprite = createCustomImage();
            File file = new File(fileName);
            if (!ImageIO.write(sprite, type, file)) throw new IOException("NO '" + type + "'image writer found ");
    ...
    protected void render(Graphics g) {
        super.render(g);
        //draw help
        g.drawString("",20,35);//super会draw FPS
