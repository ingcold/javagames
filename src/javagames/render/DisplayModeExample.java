...
public class DisplayModeExample extends JFrame {
    class DisplayModeWrapper{
        private DisplayMode dm;//alt+enter
        public DisplayModeWrapper(DisplayMode dm){this.dm=dm;}
        public boolean equals(Object obj){
            DisplayModeWrapper other=(DisplayModeWrapper)obj;
            if(dm.getWidth()!=other.dm.getWidth())return false;
            if(dm.getHeight()!=other.dm.getHeight())return false;
            return true;
        }
    ...
    private JPanel getMainPanel(){//曾经这里抄成了getMainPane导致错误
        JPanel p=new JPanel();
        displayModes=new JComboBox(listDisplayModes());
        p.add(displayModes);
        JButton enterButton=new JButton("Enter Full Screen");
        enterButton.addActionListener(new ActionListener(){
            public void actionPerformed(ActionEvent e){onEnterFullScreen();}
        });
        p.add(enterButton);//曾经在这里抄错了，导致错误
