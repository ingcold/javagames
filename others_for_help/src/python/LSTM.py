...
chars_list=sorted(list(set(text)))#text->set->tuple; sorted不改变原来，只返回sorted的元组
...
for i in range(0,len(text)-n_time):
    seq_chars.append(text[i:i+n_time])#分片
    next_chars.append(text[i+n_time])

input_data=np.zeros((len(seq_chars),n_time,n_chars),dtype=np.bool)#bool_
correct_data=np.zeros((len(seq_chars),n_chars),dtype=np.bool)#bool_
